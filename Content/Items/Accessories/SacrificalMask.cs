    ...
    public class SacrificalMask : ModItem
    {
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            if (player.ownedProjectileCounts[ModContent.ProjectileType<SacrificalDagger>()] < 8)
            {
                ...
                Projectile.NewProjectile(player.GetSource_FromAI(), player.Center, Vector2.Zero, ModContent.ProjectileType<SacrificalDagger>(), (int)player.GetTotalDamage(DamageClass.Generic).ApplyTo(60.ApplyAccArmorDamageBonus(player)), 1, player.whoAmI);
            }
